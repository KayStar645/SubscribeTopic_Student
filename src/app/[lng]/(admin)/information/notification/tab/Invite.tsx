        ...
        <div className='relative overflow-hidden p-3 border-round-xl bg-white shadow-2 flex flex-column gap-3'>
            ...
            {inviteQuery.response?.data && inviteQuery.response.data.length > 0
                ? inviteQuery.response?.data.map((invite) => (
                      <div key={invite.id} className='border-1 border-300 border-round-xl p-3 flex flex-column gap-3'>
                ...
                : !inviteQuery.isFetching && (
                      <p className='text-800 font-semibold text-center py-3'>Chưa có lời mời gửi đến bạn</p>
                  )}
        </div>
    );
};
